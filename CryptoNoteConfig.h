...
namespace CryptoNote
{
	namespace parameters
	{
		...
		const size_t CRYPTONOTE_MAX_TX_SIZE = 1000000000;
                const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX = 0x89; /* "fire" address prefix */
		const size_t CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW = 60;	
		const uint64_t DIFFICULTY_TARGET_DRGL = 81;
		const unsigned EMISSION_SPEED_FACTOR = 9;
                const unsigned EMISSION_SPEED_FACTOR_FANGO = 10;  //major version 8
                const unsigned EMISSION_SPEED_FACTOR_FUEGO = 11;   //major version 9
	...
	const int P2P_DEFAULT_PORT = 14793;
 	const int RPC_DEFAULT_PORT = 25132;
